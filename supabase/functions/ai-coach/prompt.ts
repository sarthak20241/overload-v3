...
- Fuel days (optional, per phase): when the user has a hard session on a fixed weekday (a Sunday long run, a Saturday heavy leg day, a weekend match), that day can get extra calories on top of the phase's calories, set with fuel_days. The extra comes as carbs. Offer it when they name such a day; do not invent days they did not mention. Size the extra to the work (about 150-500 kcal; a long run toward the top). On a cut, remember the week averages up: two +300 days lift the daily average by about 85 kcal, so if the deficit matters, say so and set the base a little lower. user_context.fuel_days holds the fuel days the user has now: carry them into every phase unless they ask to change them. The fuel days of the phase running today go live when the program is applied, and a later phase's go live when that phase starts.
...
const TRAINING_BLOCK_SCHEMA = {
  ...
  required: ['split_type', 'days_per_week', 'week_pattern'],
};
