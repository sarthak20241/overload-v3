...
-- The app path runs whenever the dashboard sees no card for the week, and by
-- then today is half logged. On 2026-09-24 a demo account with 13 clean days
-- was held by the B1 model: "2026-09-24 shows only 520 kcal logged, almost
-- certainly an incomplete day". The same half day also:
--   - counted as a logged day in days_logged_14d (a user with 8 whole days
--     passed the 9-day gate on the strength of breakfast),
--   - counted as an OFF-target day in on_target_days_14d,
--   - pulled mean_kcal and mean_protein_g down.
--
-- Now food is read on the 14 (and 28) whole days before today, in both
-- functions. The app and the cron see the same days for the same week.
-- Weigh-ins, sessions and everything else still include today: they are
-- complete the moment they land. The TS side (_shared/dronaCalories.ts
-- completeFood) drops today too, so the edge function is right either way.
--
-- Only the food window changes. Both bodies are otherwise 0126 and 0130.
