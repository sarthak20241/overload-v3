...
export function workoutCoachOpener(ctx: WorkoutCoachContext): string {
  if (ctx.kind === 'review') {
    return [
      ...
      "- I haven't saved this session yet, so it can still be changed. The numbers above are how you address exercises in edit_active_workout; use that tool to change anything, and don't tell me something changed unless you called it.",
  ...
  return [
    ...
    '- The numbers above are how you address exercises in edit_active_workout. Use that tool to actually change my session; saying it changed does not change it.',
