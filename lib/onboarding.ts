 ...
 * protein-first macro framing. Needs weight and age; height falls back to a
 * population median (a ~±60 kcal term, and everything is editable on the
 * nutrition screen). Returns null when the inputs were skipped, so the plan
...
export function computeDailyTargets(a: OnboardingAnswers): DailyTargets | null {
  ...
  const bmr = 10 * w + 6.25 * h - 5 * a.ageYears + genderTerm;
  const kcal = Math.max(1200, roundTo(bmr * activityFactor(a.frequency ?? 3) * goalAdjustment(a), 25));
  const protein = roundTo(w * PROTEIN_G_PER_KG[a.goal ?? 'general'], 5);
