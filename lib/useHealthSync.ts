 ...
 *
 * On app-open and each return to the foreground, push hand-typed weight, body
 * fat and measurements that have not reached the server, pull the latest hub
 * data, and recompute today's readiness for the signed-in user. Body logs go
 * first, so the hub sync's manual-wins guard already sees today's typed rows. No-op for guests /
...
export function useForegroundHealthSync(): void {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      flushBodyLogsOnOpen(supabase, userId)
        .catch(() => {})
